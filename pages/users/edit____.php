...
$userQuery = "SELECT 
                u.id, u.email, u.password, u.user_type, u.is_active, u.email_verified, u.two_factor_enabled, u.last_login, u.created_at as user_created_at, u.updated_at as user_updated_at, u.role_id,
                ur.role_name,
...
$full_name = $user['staff_full_name'] ?? $user['full_name'] ?? '';
$phone = $user['staff_phone_number'] ?? '';
...
// Ensure it's an integer if not null
if ($experience_years !== null && is_numeric($experience_years)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (canEditField('staff_info', $current_user_role, $user['user_type'], $is_self)) {
        ...
        } elseif ($date_of_joining === '0000-00-00' || $date_of_joining === 'NULL') {
            $date_of_joining = null;
        }
        ...
        // Handle empty experience years
        if (empty($experience_years) || !is_numeric($experience_years)) {
