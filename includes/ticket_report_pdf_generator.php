...
class TicketReportPDFGenerator {
    ...
    public function __construct($report_data, $filters) {
        ...
        // Initialize TCPDF with 16:9 aspect ratio (common for PowerPoint presentations)
        // Using 297mm x 167mm which approximates the 16:9 aspect ratio for better presentation
        $this->pdf = new \TCPDF('L', 'mm', [297, 167], true, 'UTF-8', false);
    ...
    private function addHeaderSection() {
        ...
        // Set Flashnet brand colors (using blue as primary)
        ...
        // Company branding with Flashnet colors
        $this->pdf->SetFont('helvetica', 'B', 22);
        $this->pdf->Cell(0, 12, 'Flashnet – Managed IT Services', 0, 1, 'C', false, '', 0, false, 'T', 'M');
        
        // Report title
        $this->pdf->SetTextColor(0, 0, 0); // Reset to black for better readability
        $this->pdf->SetFont('helvetica', 'B', 18);
        $this->pdf->Cell(0, 10, 'Ticket Report', 0, 1, 'C', false, '', 0, false, 'T', 'M');
        
        // Client name (if specified)
        if (!empty($this->filters['client_id'])) {
            $client_name = $this->getClientNameById($this->filters['client_id']);
            $this->pdf->SetFont('helvetica', 'B', 16);
            $this->pdf->SetTextColor(0, 78, 137); // Flashnet blue
            $this->pdf->Cell(0, 10, $client_name . ' - ' . date('M Y'), 0, 1, 'C', false, '', 0, false, 'T', 'M');
        } else {
            $this->pdf->SetFont('helvetica', 'B', 16);
            $this->pdf->SetTextColor(0, 78, 137); // Flashnet blue
            $this->pdf->Cell(0, 10, 'All Clients - ' . date('M Y'), 0, 1, 'C', false, '', 0, false, 'T', 'M');
        }
        
        // Date range
        $this->pdf->SetTextColor(0, 0, 0); // Reset to black
        $this->pdf->SetFont('helvetica', '', 14);
        $date_range = $this->filters['start_date'] . ' to ' . $this->filters['end_date'];
        $this->pdf->Cell(0, 8, 'Date Range: ' . $date_range, 0, 1, 'C', false, '', 0, false, 'T', 'M');
        
        // Current user info - get from session using proper function
        $current_user = getCurrentUser();
        $user_name = ($current_user['staff_profile']['full_name'] ?? $current_user['email'] ?? 'System User');
        $user_designation = ($current_user['staff_profile']['designation'] ?? ucfirst($current_user['user_type'] ?? 'User'));
        $user_info = $user_name . ' - ' . $user_designation;
        $this->pdf->Cell(0, 8, 'Generated by: ' . $user_info, 0, 1, 'C', false, '', 0, false, 'T', 'M');
        
        // Separator line
        $this->pdf->Ln(5);
        $this->pdf->Line(15, $this->pdf->GetY(), 195, $this->pdf->GetY()); // Line from left margin to right margin
        $this->pdf->Ln(5);
    }
    ...
    private function addDetailedTicketsTable() {
        ...
        $this->pdf->Cell(25, 10, 'Ticket #', 1, 0, 'C', true);
        $this->pdf->Cell(30, 10, 'Client', 1, 0, 'C', true);
        ...
        $this->pdf->Cell(20, 10, 'Assigned To', 1, 1, 'C', true);
        ...
        if (empty($tickets)) {
            ...
            foreach ($tickets as $ticket) {
                ...
                $this->pdf->Cell(25, 10, $ticket['ticket_number'] ?? $ticket['id'], 1, 0, 'C', true);
                $this->pdf->Cell(30, 10, substr($ticket['company_name'] ?? 'Internal', 0, 18), 1, 0, 'L', true);
                ...
                $this->pdf->Cell(20, 10, substr($ticket['assigned_to'] ?? 'Unassigned', 0, 15), 1, 1, 'C', true);
